'''
/home/dqwang/anaconda3/envs/ct/lib/python3.9/site-packages/transformers/trainer.py
'''
...
class Trainer:
    ...
    def _inner_training_loop(
        self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None, timestamps=None
    ):
        ...
        return TrainOutput(self.state.global_step, train_loss, metrics), timestamps

    def syn_train(self, inputs=None, keys=None, lr=None):

        def straight_through_estimator(x):
            rounded_x = torch.round(x)
            return (rounded_x - x).detach() + x
        ...
        if not delay_optimizer_creation:
            self.create_optimizer_and_scheduler(num_training_steps=args.num_train_epochs)

        self.state = TrainerState()
        self.state.is_hyper_param_search = None
        ...
        for epoch in range(0, args.num_train_epochs):
            ...
            index = random.randint(0, inputs[0].shape[-1]-1)
            input_dict = {}
            # print("input: ", input)
            for i in range(len(keys)):
                input_dict[keys[i]] = inputs[i][:, :, index]
                # print("type: ", type(input_dict[keys[i]]))
                print("keys: ", keys[i])

            # print("input_dict: ", input_dict)

            model.train()
            inputs = self._prepare_inputs(input_dict)
            ...
            with torch.no_grad:
                for param, param_grad in zip(params, grad):
                    param -= lr * param_grad
                    
            if epoch == (args.num_train_epochs - 1):
                timestamps = {name: param for name, param in model.named_parameters() if 'lora_' in name or 'bias' in name}
